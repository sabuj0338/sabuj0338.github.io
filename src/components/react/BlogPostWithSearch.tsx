...
import FormattedDateR from "./FormattedDateR";
...
const BlogPostWithSearch = ({ sortedPosts }: Props) => {
  ...
  const filteredBlogPosts = sortedPosts.filter((post: PostType) =>
    ...
    <>
      ...
      {filteredBlogPosts.length > 0 &&
        filteredBlogPosts.map((post) => (
          <a href={`/blog/${post.slug}`} className="w-full" key={post.slug}>
            <div className="w-full mb-8">
              <div className="flex flex-col justify-between md:flex-row">
                <h4 className="w-full mb-2 text-lg font-medium text-gray-900 md:text-xl dark:text-gray-100">
                  {post.title}
                </h4>
                <p className="w-full mb-4 text-left text-sm text-gray-500 md:text-right md:mb-0">
